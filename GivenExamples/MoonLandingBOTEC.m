...
V1=sqrt(mu_M/r1);
V1/1e3
disp('Orbital velocity (km/s) at 100 km of elliptical orbit with 100 km apolune and
and 20 km perilune');
V2=sqrt(mu_M*(2/r1-1/at));
V2/1e3
disp('Hohmann transfer DV (m/s) to lower altitude from 100 km circular to 20 km 
perilune');
DV_Hohmann=V1-V2;
DV_Hohmann
...
DV_hor/1e3
disp('Vertical speed gain (km/s) for a constant mass falling from 20 km in the 
Moons gravity field (constant g_Moon)');
DV_ver=sqrt(2*g_M*h2);
DV_ver/1e3
%disp('Total speed (km/s) to brake away is approximated as 
sqrt(DV_hor^2+DV_ver^2)');
%DV_tot=sqrt(DV_hor^2+DV_ver^2);
%DV_tot/1e3
disp('Total speed (km/s) to brake away is approximated as DV_hor+DV_ver)');
DV_tot=DV_hor+DV_ver;
DV_tot/1e3
disp('Required constant acceleration (m/s^2) to brake from orbital speed to zero 
speed');
a_des=DV_tot/t_des;
a_des
